...
\title{\textsc{Detecture}: VLM-Guided Multi-Texture Segmentation via Multiplexed Grounding}
...
% ============================================================================
\begin{abstract}
Texture segmentation is not simply object segmentation with different masks. Before boundaries can be drawn, a system must decide which visual regularities are perceptually major, at what scale they should be grouped, and how they should be separated when they do not align with object categories. Texture thus occupies an intermediate regime: below full semantics, yet too structured to be captured by local cues alone. We propose that this gap can be bridged through language used in a \emph{sub-semantic} way---a Vision-Language Model first produces texture-guided descriptions of appearance and spatial organisation (rather than object labels), which then serve as anchors for segmentation. Based on this idea, we present \textsc{Detecture}, an end-to-end architecture for parallel multi-texture segmentation that couples open-vocabulary reasoning with a texture-aware SAM-family decoder. In developing the system we identify five architectural pathologies that prevent standard \texttt{[SEG]}-token pipelines from scaling to multi-target texture grounding: \emph{Context Leakage} (causal attention contaminates downstream \texttt{[SEG]} representations, inflating inter-texture cosine similarity from 0.16 to 0.74 under shared context), \emph{Slot-1 Positional Bias} (SAM's multi-prompt cross-attention allocates 90.5\,\% of pixels to the first query regardless of semantic content), the \emph{Count / Language Collapse} dilemma (uniform LM loss terminates generation after one texture, while binary masking causes catastrophic linguistic forgetting), \emph{Directional Drift} (an over-parameterised projector memorises domain-specific manifold directions), and \emph{Bottleneck Semantic Squash} (projecting a 4096-dim VLM grounding representation through a 256-dim square bottleneck destroys the semantic richness a pretrained segmentation backbone can already interpret). A parallel Zero-Shot experiment reframes the projector's role: SAM~3 with a hand-crafted, object-aware text prompt reaches \textbf{0.928 mIoU on RWTD without any training}, suggesting the projector should \emph{preserve} SAM's native semantic mapping rather than relearn one. Detecture instantiates this principle through a \textbf{Bridge-to-Native-Space projector} that maps \texttt{[SEG]} into SAM~3's native 1024-dim text-encoder width and reuses SAM~3's own frozen $1024 \to 256$ resizer, \textbf{block-diagonal attention masking} and \textbf{batch-multiplexed SAM decoding} that decouple multi-query grounding, and a \textbf{Shifted-Zero LM-loss cliff} paired with \textbf{masked-row trainable \texttt{[SEG]} rows} (warm-started from \texttt{<|im\_end|>}) that together also cure a subtle emission-failure mode which a na\"ive $\lambda(\text{SEG}) = 0$ cliff silently induces. With a permanently frozen SAM~3 and a two-stage curriculum, the method uses only ${\sim}8.2$M trainable parameters on top of frozen Qwen3-VL-8B and SAM~3 backbones. We further contribute the \textbf{Detecture Miner}, a geometry-first texture mining framework, and adapt the \textbf{Real-World Texture Dataset (RWTD)} for VLM-guided evaluation.
\end{abstract}
...
This creates a central paradox. Modern multimodal models are linguistic, and language is usually treated as a vehicle for semantics. Texture, by contrast, has often been framed as a pre-semantic or even non-semantic phenomenon. At first glance, this seems to make VLMs the wrong tool for the problem. Our starting point is the opposite hypothesis: language may become useful for texture precisely when it is used \emph{below} the level of object naming. The key question is not whether a model can say ``this is grass'' or ``this is a building,'' but whether it can generate \emph{sub-semantic texture descriptions} such as \emph{dense thin green vertical strands}, \emph{weathered grey granular surface}, or \emph{coarse irregular brown ridges in the lower foreground}. These descriptions are verbal, but they are not primarily object labels. They act as anchors for perceptual regularities that otherwise remain difficult to stabilise across images.
...
This framing also explains why prior progress, while important, remains incomplete. TextureSAM~\cite{texturesam} showed that SAM-family models carry a systematic shape bias and that this bias can be shifted toward texture sensitivity through texture-augmented fine-tuning. That result established that \emph{what} to segment in texture-dominant scenes is, at least in part, learnable. But it did not resolve the more fundamental upstream problem: \emph{how many} textures should be segmented, \emph{which} ones are perceptually major, and \emph{how} they should be described in a way that survives ambiguity and domain shift. A prompt-driven segmentation model, even a texture-aware one, still requires these decisions to be supplied externally. What is missing is an autonomous mechanism for discovering and expressing texture identity before mask prediction.

We therefore propose \textsc{Detecture}, an end-to-end architecture for parallel multi-texture segmentation that couples a VLM's open-vocabulary reasoning with a texture-aware SAM-family decoder. The VLM first extracts texture-guided descriptions that are linguistic yet intentionally non-object-centric, and dedicated \texttt{[SEG]} representations derived from these descriptions are then projected into the segmentation decoder to produce multiple masks in a single forward pass. This design turns language into a sub-semantic coordination space between two otherwise mismatched systems: one that can describe, and one that can localise. It also offers a practical advantage for hard cases. Because texture descriptions can abstract over exact appearance, they provide a more stable anchor when the image comes from unusual viewpoints, expert domains, or distributions far from the object-centric data on which most foundation models were trained.

Making this idea work required more than simply attaching a VLM to SAM. As soon as we moved from single-target grounding to autonomous multi-texture partitioning, we encountered five architectural pathologies. \emph{Context Leakage} caused later \texttt{[SEG]} tokens to inherit semantic residue from earlier texture descriptions under standard causal attention. \emph{Slot-1 Positional Bias} in SAM's decoder forced nearly all pixels toward the first prompt slot, preventing fair competition among multiple texture queries. The \emph{Count\,/\,Language Collapse} dilemma emerged during co-training: uniform language-modeling loss terminated generation after a single description, while binary masking on the text produced catastrophic forgetting of Qwen's linguistic prior. \emph{Directional Drift} appeared when a high-capacity projector overfit domain-specific directions rather than learning a transferable mapping between VLM and decoder spaces. And \emph{Bottleneck Semantic Squash} emerged once drift was cured: projecting Qwen's rich 4096-dim grounding representation through a 256-dim square bottleneck destroyed the semantic richness SAM's pretrained text pathway already understood---a ceiling we confirmed directly by running a parallel Zero-Shot experiment that bypassed the VLM and projector entirely and reached \textbf{0.928 mIoU on RWTD} with a hand-crafted object-aware text prompt. These failures are not incidental engineering bugs. They reflect a deeper fact: once texture is treated as an open-ended, multi-region perceptual problem, the standard assumptions behind current \texttt{[SEG]}-token grounding pipelines no longer hold.

Detecture resolves these pathologies through a matched set of design choices. A \textbf{block-diagonal attention mask} isolates each texture block during \texttt{[SEG]} extraction, eliminating cross-texture contamination. \textbf{Batch-multiplexed SAM decoding} reshapes the query tensor so each texture occupies its own independent ``Slot~1'' in a batched forward pass, bypassing the positional bias entirely. A \textbf{Shifted-Zero LM-loss cliff}---assigning loss weight $1.0$ at the \texttt{[SEG]} position (to supervise emission) and $0.0$ one position forward (to leave the grounding embedding geometrically free)---resolves the Count\,/\,Language dilemma, while \textbf{masked-row trainable \texttt{[SEG]} rows} warm-started from \texttt{<|im\_end|>} calibrate the output projection so the model actually emits \texttt{[SEG]} at inference. A \textbf{Bridge-to-Native-Space projector} maps the 4096-dim \texttt{[SEG]} hidden state into SAM~3's native 1024-dim text-encoder width and hands the final $1024 \to 256$ step back to SAM~3's own pretrained resizer, kept frozen---turning cross-model alignment from a \emph{learning} problem into a \emph{shape-matching} problem, and simultaneously curing Directional Drift and Bottleneck Semantic Squash. Together with a permanently frozen SAM~3 and a two-stage curriculum, these mechanisms enable parallel multi-texture segmentation in a single forward pass with only ${\sim}8.2$M trainable parameters on top of frozen Qwen3-VL-8B and SAM~3 backbones.

The paper also contributes a full data ecosystem around this idea. Building on TextureSAM's texture-augmented ADE20K, we introduce the \textbf{Detecture Miner}, a geometry-first mining framework that identifies texture-rich scenes, extracts distinct texture regions, and associates them with VLM-generated texture descriptions. We further adapt the \textbf{Real-World Texture Dataset (RWTD)}~\cite{texturesam} for VLM-guided evaluation, using it not only as a benchmark but as a stress test whose distributional distance helped reveal several of the architectural failures above. In this sense, the paper is organised around a joint thesis: texture understanding requires both a new representation and a new pipeline, where linguistic texture anchoring and segmentation are co-designed rather than treated as separate modules.

The central question of this work is therefore the following:

\begin{quote}
\textbf{Can language function as a sub-semantic anchor for texture, enabling a model to identify, describe, and segment multiple texture regions in a single coherent forward pass?}
\end{quote}

We answer this question affirmatively through a unified architecture and data pipeline. Our contributions are:

\begin{itemize}
    \item We reframe multi-texture segmentation as a fundamental representational problem, not merely a prompt-engineering or decoder-integration problem, and propose \emph{sub-semantic linguistic anchoring} as the missing bridge between texture perception and pixel grounding.

    \item We introduce \textsc{Detecture}, a VLM-to-SAM architecture for parallel multi-texture segmentation, and systematically identify five pathologies that arise in the multi-target regime---\emph{Context Leakage}, \emph{Slot-1 Positional Bias}, the \emph{Count\,/\,Language Collapse} dilemma, \emph{Directional Drift}, and \emph{Bottleneck Semantic Squash}---plus a subtle \emph{Emission Failure} mode that emerges while resolving the Count\,/\,Language dilemma.

    \item We propose a corresponding set of mechanisms---\textbf{block-diagonal attention masking}, \textbf{batch multiplexing}, the \textbf{Shifted-Zero LM-loss cliff with masked-row trainable grounding rows}, and a \textbf{Bridge-to-Native-Space projector} that reuses SAM~3's own frozen pretrained resizer---that together enable stable one-pass multi-texture grounding with only ${\sim}8.2$M trainable parameters and a permanently frozen SAM~3.

    \item We contribute the \textbf{Detecture Miner} for mining and annotating texture-rich training data, and adapt \textbf{RWTD} as an external benchmark for evaluating generalisation under real-world texture ambiguity. The cross-domain gap between training (ADE20K) and evaluation (RWTD) was itself instrumental in surfacing several of the pathologies above---evidence that architecture and data benefit from being co-designed.
\end{itemize}
...
% 4. DATA ECOSYSTEM
% ============================================================================
\section{Data Ecosystem: The Detecture Miner and Dual-Evaluation Strategy}
\label{sec:data}
...
We introduce the \textbf{Detecture Miner}, an automated, scalable framework designed to transform generic semantic datasets into high-quality texture-grounding data. The pipeline operates in three stages: intelligent filtering, geometric extraction, and VLM-based annotation.
